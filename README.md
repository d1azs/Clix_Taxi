...
<p align="center">
  ...
  <i>Дипломний проєкт з розробки кросплатформного мобільного додатку</i>
</p>
...
* 📱 **Автоматична збірка APK (GitHub Actions)**: Завантажити свіжий файл `clix-taxi-app-release.apk` можна у розділі **Actions** → **Build Flutter APK** цього репозиторію.
...
1. **📱 Кросплатформний мобільний додаток (Flutter)** — єдиний клієнтський застосунок, що адаптується під роль користувача (пасажир / водій / диспетчер) на основі його облікового запису.
2. **⚙️ Високопродуктивний бекенд (Django + Django Channels)** — серверна частина на базі протоколів REST API та WebSockets, що обслуговує запити, зберігає дані у PostgreSQL та забезпечує миттєвий обмін координатами й статусами замовлень.
...
```
                                 ...
                                 │    Пасажир  ·  Водій  ·  Диспетчер                     │
                                 └──────────┬──────────────────────────┬──────────────────┘
                                            │                          │
                                            │ HTTP (REST API / JWT)    │ WebSockets (Real-time)
                                            ▼                          ▼
                                 ┌────────────────────────────────────────────────────────┐
...
### 🖥️ Функціонал Диспетчера
* Перегляд та управління замовленнями в реальному часі.
...
* Статистичний дашборд: кількість активних замовлень, загальна виручка та аналітика.
...
| **Лендінг** | React 18 / Vite | Промо-сторінка проєкту |
